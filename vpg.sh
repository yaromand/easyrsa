...
if [[ ! -e /etc/openvpn/server.conf ]]; then
	...
	if [[ $(ip -4 addr | grep inet) -eq 1 ]]; then
		ip=$(ip -4 addr | grep inet | grep -vE | cut -d '/' -f 1 | grep -oE '[0-9]{1,3}(\.[0-9]{1,3}){3}')
	else
...
echo "iptables -t nat -A POSTROUTING -s 10.8.0.0/24 -o ens3 -j MASQUERADE
# new
...
verb 3
crl-verify /etc/openvpn/server/crl.pem" >> /etc/openvpn/server.conf
...
[Service]
Type=oneshot
ExecStart=$iptables_path -t nat -A POSTROUTING -s 10.8.0.0/24 -o ens3 -j MASQUERADE
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo "Finished!"
	echo
	echo "The client configuration is available in:" ~/"$client ($ip) .conf"
	...
	case "$option" in
		...
		1)
			...
			new_client
			echo
			echo "$client ($ip) added. Configuration available in:" ~/"$client ($ip).conf"
			exit
		;;
		2)

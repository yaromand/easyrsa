...
function installQuestions() {
	...
	SERVER_PUB_IP=$(grep -m 1 -oE '^[0-9]{1,3}(\.[0-9]{1,3}){3}$' <<< "$(wget -T 10 -t 1 -4qO- "http://ip1.dynupdate.no-ip.com/" || curl -m 10 -4Ls "http://ip1.dynupdate.no-ip.com/")")
	...
	until [[ ${CLIENT_DNS_2} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		read -rp "Second DNS resolver to use for the clients (optional): " -e -i 77.88.8.1 CLIENT_DNS_2
...
Endpoint = ${ENDPOINT}
AllowedIPs = 0.0.0.0/0,::/0" >>"${HOME_DIR}/${ENDPOINT}-client-${CLIENT_NAME}.conf"
...
AllowedIPs = ${CLIENT_WG_IPV4}/32,${CLIENT_WG_IPV6}/128" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	...
	echo -e "\nHere is your client config file as a QR Code:"
	 ...
	 qrencode -t ansiutf8 -l L <"${HOME_DIR}/${ENDPOINT}-client-${CLIENT_NAME}.conf"

	echo "It is also available in ${HOME_DIR}/${ENDPOINT}-client-${CLIENT_NAME}.conf"
}
...
do
    ...
    #CLIENT_NAME=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
	...
	# Home directory of the user, where the client configuration will be written
	if [ -e "/root/${CLIENT_NAME}" ]; then
		# if $1 is a user name
		HOME_DIR="/root/${CLIENT_NAME}"
	elif [ "${SUDO_USER}" ]; then
		# if not, use SUDO_USER
		if [ "${SUDO_USER}" == "root" ]; then
			# If running sudo as root
...
PresharedKey = ${CLIENT_PRE_SHARED_KEY}
Endpoint = ${ENDPOINT}
AllowedIPs = 0.0.0.0/0,::/0" >>"${HOME_DIR}/client-${CLIENT_NAME}.conf"
